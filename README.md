...
**Live scripts, `uifigure`, and `yyaxis`** - some live/UI figure export paths need to copy figure contents into a temporary legacy figure before exporting. MATLAB does not fully support copying axes that use multiple coordinate systems, such as axes created with `yyaxis`, so these figures can export as empty output or emit warnings such as `Some uifigure controls cannot be exported...` or `Object Copy of Axes with multiple coordinate systems is not supported`. If possible, create the plot in a standalone `.m` script using a legacy `figure`. For affected live/UI cases, try MATLAB's built-in `exportgraphics` function or adjust axes layout properties such as `OuterPosition` and `LooseInset` before exporting. See issue [#387](https://github.com/altmany/export_fig/issues/387) for examples and discussion.
